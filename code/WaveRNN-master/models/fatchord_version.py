...
class UpsampleNetwork(nn.Module):
    def __init__(self, feat_dims, upsample_scales, compute_dims,
        ...
        total_scale = np.cumproduct(upsample_scales)[-1]   # 求内积，80
        self.indent = pad * total_scale         # 550
...
class WaveRNN(nn.Module):
    def __init__(self, rnn_dims, fc_dims, bits, pad, upsample_factors,
        ...
        self.aux_dims = res_out_dims // 4     # res_out_dim=128
    ...
    def forward(self, x, mels):
        ...
        self.step += 1
        bsize = x.size(0)
        h1 = torch.zeros(1, bsize, self.rnn_dims, device=device)   # 生成1*bsize*rnn_dims的张量，rnn_dims=512
        h2 = torch.zeros(1, bsize, self.rnn_dims, device=device)   # 生成1*bsize*rnn_dims的张量，rnn_dims=512
        mels, aux = self.upsample(mels)     # [32, 1375, 37], [32, 1375, 128]


        aux_idx = [self.aux_dims * i for i in range(5)]
        a1 = aux[:, :, aux_idx[0]:aux_idx[1]]    # [32,1375,32]
        a2 = aux[:, :, aux_idx[1]:aux_idx[2]]    # [32,1375,32]
        a3 = aux[:, :, aux_idx[2]:aux_idx[3]]    # [32,1375,32]
        a4 = aux[:, :, aux_idx[3]:aux_idx[4]]    # [32,1375,32]
    ...
    def generate(self, mels, save_path: Union[str, Path], batched, target, overlap, mu_law):
        ...
        with torch.no_grad():
            ...
            wave_len = (mels.size(-1) - 1) * self.hop_length   # mel*80   这里在用sp+f0训练时需要考虑是否要除以3.42
