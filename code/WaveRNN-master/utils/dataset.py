...
data_train = "sp+f0_speaker_to_fine_tuning"
data_pkl = "dataset_sp_f0_speaker_finr_tuning.pkl"
quant = "quant_speaker_to_fine_tuning"

class VocoderDataset(Dataset):
    def __init__(self, path: Path, dataset_ids, train_gta=False):
        self.metadata = dataset_ids
        self.mel_path = path/'gta' if train_gta else path/data_train
...
def get_vocoder_datasets(path: Path, batch_size, train_gta):
    ...
    with open(path/data_pkl, 'rb') as f:
...
class TTSDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mel = np.load(self.path/data_train/f'{item_id}.npy')
        mel_len = mel.shape[-1]
