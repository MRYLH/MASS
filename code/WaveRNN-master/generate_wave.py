...
def gen_testset(model: WaveRNN, test_set_wav, samples, batched, target, overlap, save_path: Path):
    '''
    :param model:
...
if __name__ == "__main__":
    ...
    voc_weights = "/wave_step800K_weights.pyt"
    # output_file = "/home/NewDisk/yelinhui/yelinhui/Wavenet/wavenet_vocoder-master/delate"
    ...
    parser.add_argument('--voc_weights', '-w', type=str, default = voc_weights, help='[string/path] Load in different WaveRNN weights')
    ...
    if args.target is None:
        args.target = hp.voc_target    # 11000
    if args.overlap is None:           # 550
        args.overlap = hp.voc_overlap
    if args.batched is None:           # True
    ...
    if file:
        ...
        test_set_path = "/emotion_npy"
        gen_testset(model, test_set_path, samples, batched, target, overlap, paths.voc_output)
