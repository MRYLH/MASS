...
def world_decompose(wav, fs, frame_period=5.0):
    ...
    f0, timeaxis = pyworld.harvest(wav, fs, frame_period=frame_period, f0_floor=71.0,
                                   f0_ceil=800.0)
...
def world_speech_synthesis_need_decode(f0, decoded_sp, ap, fs, frame_period):
    min_len = min([len(f0), len(decoded_sp), len(ap)])
    f0 = f0[:min_len]
    ap = ap[:min_len]
    wav = pyworld.synthesize(f0, decoded_sp, ap, fs, frame_period)
    wav = wav.astype(np.float32)
...
def pitch_conversion(f0, mean_log_src, std_log_src, mean_log_target, std_log_target):
    ...
    f0_converted = np.exp((np.ma.log(
        f0) - mean_log_src) / std_log_src * std_log_target + mean_log_target)
...
def convert_continuos_f0(f0):
    ...
    if (f0 == 0).all():
        logging.warn("all of the f0 values are 0.")  # abc
        return uv, f0
    start_f0 = f0[f0 != 0][0]
    end_f0 = f0[f0 != 0][-1]

    # padding start and end of f0 sequence
    start_idx = np.where(f0 == start_f0)[0][
        0]
