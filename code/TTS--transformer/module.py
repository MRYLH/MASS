...
class PostConvNet(nn.Module):
    ...
    def forward(self, input_, mask=None):
        ...
        input_ = self.conv2(input_)[:, :, :-4]    # 最后四列不要？？？
        return input_
...
class MultiheadAttention(nn.Module):
    ...
    def forward(self, key, value, query, mask=None, query_mask=None):
        # Get attention score
        attn = t.bmm(query, key.transpose(1, 2))     # 表示矩阵乘法
...
class Attention(nn.Module):
    ...
    def __init__(self, num_hidden, h=4):
        ...
        self.num_hidden_per_attn = num_hidden // h     # //表示整除
        self.h = h
    ...
    def forward(self, memory, decoder_input, mask=None, query_mask=None):
        ...
        # Make multihead
        key = self.key(memory).view(batch_size, seq_k, self.h, self.num_hidden_per_attn)    # view类似于reshape
        ...
        value = value.permute(2, 0, 1, 3).contiguous().view(-1, seq_k, self.num_hidden_per_attn)    # permute类似于transpose
