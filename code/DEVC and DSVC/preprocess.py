...
def get_spk_world_feats(spk_fold_path, mc_dir_train, mc_dir_test, sample_rate=16000):
    ...
    for wav_file in tqdm(train_paths):
        ...
        normed_coded_sp = normalize_coded_sp(coded_sp, coded_sps_mean, coded_sps_std)   # normed_coded_sp = (coded_sp - coded_sp_mean) / coded_sp_std
...
if __name__ == '__main__':
    ...
    origin_wavpath_default = ""   # Your dataset
    target_wavpath_default = ""    # the directory to save processed wav
    mc_dir_train_default = ''   # the directory to save training data
    mc_dir_test_default = ''    # the directory to save test data
    ...
    speaker_used = ['p'+i for i in speaker_used]   # ['p262', 'p272', 'p229', 'p232', 'p292', 'p293', 'p360', 'p361', 'p248', 'p251']
    ...
    work_dir = target_wavpath     # 16k的目标数据
