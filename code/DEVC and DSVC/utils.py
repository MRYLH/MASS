...
def world_encode_wav(wav_file, fs, frame_period=5.0, coded_dim=36):
    ...
    f0, timeaxis, sp, ap = world_decompose(wav=wav, fs=fs, frame_period=frame_period)   # fs是采样率 frame_period是连续帧之间的间隔
    coded_sp = world_encode_spectral_envelop(sp = sp, fs = fs, dim = coded_dim)   # 对频谱包络进行编码  降低其维度  coded_dim是降为后的维度
...
def wav_padding(wav, sr, frame_period, multiple = 4):
    ...
    assert wav.ndim == 1 
    num_frames = len(wav)   # 获取帧数
    num_frames_padded = int((np.ceil((np.floor(num_frames / (sr * frame_period / 1000)) + 1) / multiple + 1) * multiple - 1) * (sr * frame_period / 1000)) # np.ceil 向上取整
    num_frames_diff = num_frames_padded - num_frames
    num_pad_left = num_frames_diff // 2
    num_pad_right = num_frames_diff - num_pad_left
    wav_padded = np.pad(wav, (num_pad_left, num_pad_right), 'constant', constant_values = 0)   # 数组填充
